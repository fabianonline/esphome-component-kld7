...
from esphome.const import CONF_ID, CONF_TYPE, STATE_CLASS_MEASUREMENT, CONF_STATE_CLASS, DEVICE_CLASS_PRESENCE
...
async def to_code(config):
    ...
    if detection_config := config.get(CONF_TYPE_DETECTION):
        sens = await binary_sensor.new_binary_sensor(detection_config)
        Kld7.add(var.register_detection_sensor(sens))
